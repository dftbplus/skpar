...
class PSOTest(unittest.TestCase):
    ...
    def test_pso(self):
        ...
        def evaluate(parameters, iteration, refdata=refdata, weights=refweights):
            ...
            fitness = np.atleast_1d(
                np.sqrt(sum(weights * np.power(errors / refdata, 2)))
            )
        ...
        pso = PSO(
            prange, evaluate, npart=npart, objective_weights=objectives, ngen=ngen
        )
        ...
        LOGGER.debug(
            "Fitted coefficients: {}".format(
                ", ".join(["{0:.3f}".format(par) for par in swarm.gbest.renormalized])
            )
        )

        nptest.assert_allclose(swarm.gbest.renormalized, coef, rtol=0.1, verbose=True)
...
class ParticleTest(unittest.TestCase):
    ...
    def test_create_particle(self):
        ...
        self.assertTrue(-1.0 <= p1[0] <= 1.0)
        print("\nCreated P1: {}".format(prange))
        print(pformat(p1))
        # 2D particle
        prange = [[0, 2], [-3, 3]]
        p2 = createParticle(prange)
        self.assertTrue(-1.0 <= p2[0] <= 1.0)
        self.assertTrue(-1.0 <= p2[1] <= 1.0)
    ...
    def test_evolve_strict_bounds(self):
        ...
        self.assertTrue(all([-1.0 <= pp <= 1.0 for pp in p1]))
        ...
        self.assertTrue(all([-1.0 <= pp <= 1.0 for pp in p1]))
        self.assertTrue(
            all([rr[0] <= pp <= rr[1] for pp, rr in zip(p1.renormalized, prange)])
        )

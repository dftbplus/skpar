...
class EvaluatorTest(unittest.TestCase):
    ...
    def test_evaluator_init_dflt(self):
        ...
        taskdict = {"t1": printwrapper, "t2": printwrapper}
        parnames = ["p0"]
        evaluator = ev.Evaluator(objvs, tasklist, taskdict, parnames)
        nptest.assert_array_equal(evaluator.weights, np.array([0.5, 0.5]))
        params, iteration = [2.0], 1
    ...
    def test_evaluator_utopia(self):
        ...
        taskdict = {"t1": printwrapper, "t2": printwrapper}
        parnames = ["p0"]
        evaluator = ev.Evaluator(objvs, tasklist, taskdict, parnames, utopia=np.ones(2))
        nptest.assert_array_equal(evaluator.weights, np.array([0.5, 0.5]))
        par, ii = [2.0], 1

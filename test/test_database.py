...
class DBTest(unittest.TestCase):
    ...
    def test_database(self):
        ...
        self.assertDictEqual(database.all(), {"m1": {"i1": 33}, "m2": {"i1": 10}})
        #
        database.update("m2", {"i2": 20})
        self.assertDictEqual(
            database.all(), {"m1": {"i1": 33}, "m2": {"i1": 10, "i2": 20}}
        )
        update(database, "m1", {"i2": 25})
        self.assertDictEqual(
            database.all(), {"m1": {"i1": 33, "i2": 25}, "m2": {"i1": 10, "i2": 20}}
        )
        database.update({"m2": {"i3": 40}})
        self.assertDictEqual(
            database.all(), {"m1": {"i1": 33, "i2": 25}, "m2": {"i3": 40}}
        )
        #
        mdb = database.get("m2")

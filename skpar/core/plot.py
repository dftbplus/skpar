...
def skparplot(
    ...
    **kwargs
    ...
    set_axes(
        ...
        xticklabels,
        yticklabels,
        extend_xticks=extend_xticks,
        extend_yticks=extend_yticks,
    )
...
def plot(data, weights=None, figsize=(6, 7), outfile=None, Erange=None, krange=None):
    ...
    if weights is not None and len(np.unique(weights)) - 1 > 0:
        color = cm.jet(
            (weights - np.min(weights)) / (np.max(weights) - np.min(weights))
        )
    else:
        color = ["b"] * nb

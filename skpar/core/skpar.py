...
class SKPAR:
    ...
    def __init__(self, infile="skpar_in.yaml", verbose=True):
        ...
        self.logger = get_logger(name="skpar", filename="skpar.log", verbosity=loglevel)
        ...
        taskdict, tasklist, objectives, optimisation, config = parse_input(
        ...
        self.evaluator = Evaluator(
            objectives, tasklist, taskdict, parnames, config, verbose=verbose
        )
        ...
        if optimisation is not None:
            ...
            self.optimiser = Optimiser(
                algo, parameters, self.evaluator, options, verbose=True
            )
        else:

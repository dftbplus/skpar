...
def get_type(n_models, ref, dflt_type="values"):
    ...
    if (
        n_models == 1
        and ref.ndim == 1
        and ref.dtype == [("keys", "S15"), ("values", "float")]
    ):
...
class Objective(object):
    ...
    def get(self):
        ...
        """
        #
        assert self.model_data.shape == self.ref_data.shape, "{} {}".format(
            self.model_data.shape, self.ref_data.shape
        )
        assert self.model_data.shape == self.subweights.shape, "{} {}".format(
            self.model_data.shape, self.subweights.shape
        )
        #
    ...
    def summarise(self):
        ...
        s.append(
            "{:9s}{:<15s}: {}".format(
                ...
                "Reference data",
                np.array2string(
                    self.ref_data, precision=3, suppress_small=True, max_line_width=100
                ),
            )
        )
        s.append(
    ...
    def __repr__(self):
        ...
        if hasattr(self, "model_weights"):
            s.append(
                "{:9s}{:<15s}: {}".format(
                    "", "Model weights", arr2s(self.model_weights)
                )
            )
        s.append("{:9s}{:<15s}: {}".format("", "Reference data", arr2s(self.ref_data)))
        if hasattr(self, "subweights"):
            s.append(
                "{:9s}{:<15s}: {}".format("", "Sub-weights", arr2s(self.subweights))
            )
        # s.append ("Options:\n{}".format(pformat(self.options)))
        if hasattr(self, "Model_data"):
            s.append(
                "{:9s}{:<15s}: {}".format("", "Model data", arr2s(self.model_data))
            )
        s.append(
            "{:9s}{:<15s}: {:s} / {:s}".format(
                "", "Cost/Err. func.", self.costf.__name__, self.errf.__name__
            )
        )
...
def get_refval_1d(array, align, ff={"min": np.min, "max": np.max}):
    ...
    """
    assert isinstance(align, int) or align in [
        "min",
        "max",
...
def get_refval(bands, align, ff={"min": np.min, "max": np.max}):
    ...
    assert isinstance(
        align[0], int
    ), '"align" must be (int,int) or (int, "min" or "max").'
    # Transform indexing to python-style, counting from 0, assuming
    # 'align' came from user specification, fortran-compatible, counting from 1
    iband = align[0] - 1
    try:
...
def get_refdata(data):
    ...
    if isinstance(data, dict):
        if "file" in data.keys():
            ...
            if "dtype" in loader_args.keys() and "names" in loader_args["dtype"]:
                loader_args["unpack"] = False
            # read file
            try:
                ...
                # `file` was not understood
                print("Reference data file {} cannot be found".format(file))
                raise
            ...
            try:
                ...
                return_data = np.array(
                    [(key, val) for key, val in data.items()], dtype=dtype
                )
            except TypeError:

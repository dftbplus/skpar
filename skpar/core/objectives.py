...
from skpar.core.refdata import get_refdata
...
class ObjValues(Objective):
    ...
    def __init__(self, spec, **kwargs):
        ...
        num_models = len(self.model_weights) # number of models
        self.num_models = num_models
        # coerce ref-data to 1D array if it is extracted from a 2D array
        if self.ref_data.ndim == 2 and self.ref_data.shape == (1,num_models):
            self.ref_data = self.ref_data.reshape((num_models,))
        ...
        if subweights is not None:
            self.subweights = parse_weights(subweights,
                    refdata=self.ref_data, nn=num_models,
...
def get_objective(spec, ref_db, **kwargs):
    ...
    # spec['ref'] should now be a unique ID for a reference item in a DB.
    spec['ref_data'] = get_refdata(spec['ref'], ref_db)
    if isinstance(m_names, str):
        num_models = 1
    else:
        num_models = len(m_names)
    spec['type'] = spec.get('type', get_type(num_models, spec['ref_data']))
    #   print (spec['type'], spec['query'])
    objv = objectives_mapper.get(spec['type'], ObjValues)(spec, **kwargs)
    #print (objv)
    return objv

def set_objectives(spec, ref_db, verbose=True, **kwargs):
    ...
    for item in spec:
        objv = get_objective(item, ref_db, **kwargs)

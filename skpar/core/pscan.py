...
def create_positions(ranges, numpts):
    ...
    _positions = np.vstack(
        map(np.ravel, grid)
    )  # this creates list of lists (one per direction)
...
def pformat(point):
    ...
    ss = []
    ss.append(
        "Position:  {}".format(" ".join(["{:<7.3f}".format(item) for item in point]))
    )
...
def create_population(positions):
    ...
    population = creator.Population(
        [create_point(positions, ind) for ind in range(len(positions))]
    )
    population.inext = 0
    return population
...
class PSCAN(object):
    ...
    def report(self):
        ...
        self.logger.info(
            "Best fitness     : {}".format(self.population.best.fitness.values)
        )
        if self.parnames:
            self.logger.info(
                "Best parameters:\n"

...
def plot_bs(
    ...
    **kwargs
    ...
    set_axes(ax, xlabel, ylabel, xticklabels, yticklabels, extend_xticks=True)
    ...
    if linelabels:
        ...
        if issety and len(linelabels) < len(yy):
            ...
            module_logger.warning(
                "Missing line labels: {} needed (one per set) but {} found".format(
                    len(yy), len(linelabels)
                )
            )
...
def magic_plot_bs(xval, yval, filename=None, **kwargs):
    ...
    if shift:
        module_logger.debug("Including band-gap in BS plot; shift: {}".format(shift))
        for i, s in shift:
            yy.append(yval[i] + s)
        ...
        yy = yval
    module_logger.debug(
        "Calling plot_bs with len(xval)={} and len(yval)={}".format(len(xx), len(xx))
    )

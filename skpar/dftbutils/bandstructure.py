...
def set_bands_parser(parser=None):
    ...
    if parser is None:
        parser = argparse.ArgumentParser(description="Calculate band-structure.")
        subparser = False
    else:
        subparser = True
    ...
    parser.add_argument(
        ...
        action="store",
        type=json.loads,
        help="Lattice info, e.g.:" '{"type": "TET", "param": [5.43, 100.0]}',
    )
    parser.add_argument(
        "-wd",
        dest="workdir",
    ...
    parser.add_argument(
        ...
        const="dp_dos",
        action="store",
        help="(dflt: dp_dos) Tool to convert the output to " "DoS file for plotting ",
    )
    if subparser:
...
def main_bands(args):
    ...
    logger = get_logger(
        name="dftbutils", filename="dftbutils.bands.log", verbosity=loglevel
    )
    ...
    if not args.plot_only:
        ...
        if args.dos:
            execute(
                cmd=[dos, "band.out", "dos_total.dat"], workdir=sccdir, outfile=bandslog
            )
        ...
        execute(cmd=[bands, "band.out", "bands"], workdir=bsdir, outfile=bandslog)
    ...
    if args.plot or args.plot_only:
        ...
        get_bandstructure(
            implargs, database, bsdir, "dftb", latticeinfo=args.latticeinfo
        )

...
def analyseBS_Si(bands,logger,kLinesDict,nVBtop,SOfactor=1):
    """
    Analyses the bands argument, and returns a dictionary in the same format as
    the reference data for Si, but populated with the information obtained from
...
# (see Cardona, PhysRev 1966, and my own derivation, which confirms the values)
    ...
    calc_Si = dict(list(calcEk.items()) + list(calcEgap.items()) + list(calcEk_st.items()) + list(calcmeff.items()))

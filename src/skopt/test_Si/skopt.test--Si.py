...
def main():
    ...
    # -------------------------------------------------------------------
    workdir = os.path.expanduser('~/Dropbox/Projects/SKgen/skopt/src/skopt/test_Si')
    ...
    # -------------------------------------------------------------------
    skfdir = "SKF"
    skopt_in = "test.Si--skdefs.template"
...
#                            Erange=(-12, 6)))
    ...
    maxErr = [abs(relerr * val / 100.) for relerr, val in zip(maxRelErr, refdata.values())]
    weights = [1 / err for err in maxErr]
    weights = np.asarray(weights) / np.sum(weights)
    weights = OrderedDict(zip(refkeys.split(), weights))
    ...
    s1keys = ['{s}.{k}'.format(s=s1.name, k=key) for key in s1.refdata.keys()]
